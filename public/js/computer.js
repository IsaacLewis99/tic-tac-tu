...
const displayController = (() => {
  ...
  welcomeMessage.innerHTML = "Select your mode:";
  easyComputer.id = "easy-computer";
  easyComputer.innerHTML = "Bot (Beginner)";
  easyComputer.addEventListener('click',function (e){
    displayName(1);
    name2input.value = "Bot (Beginner)";
    gameBoard.setMode(1);
  })
  hardComputer.id = "hard-computer";
  hardComputer.innerHTML = "Bot (Advanced)";
  hardComputer.addEventListener('click',function (e){
    displayName(1);
    name2input.value = "Bot (Advanced)";
    gameBoard.setMode(2);
  })
  ...
  resetGame.innerHTML = "CHANGE DIFFICULTY";
...
const gameBoard =  (() => {
    ...
    for(let j = 0; j<3;j++){
      ...
      content.addEventListener('click',function (e){
        if(e.target.innerHTML=="" && !victory){
          ...
          if (playerOneTurn) {
            e.target.innerHTML = '<div class="letter">o</div>';
  ...
  const easyMove = () =>{
    ...
    buttons[Math.floor(squaresLeft[dummy]/3)][squaresLeft[dummy]%3].innerHTML = '<div class="letter">x</div>';
  ...
  const hardMove = () =>{
    ...
    buttons[Math.floor(move/3)][move%3].innerHTML= '<div class="letter">x</div>';
  ...
  const minimax = (buttonsInput, currState, depth, turn) => {
    ...
    for(let i = 0; i<currState.length; i++){
      if(!turn) buttonsInput[Math.floor(dummy[i]/3)][dummy[i]%3].innerHTML= '<div class="letter">o</div>';
      else buttonsInput[Math.floor(dummy[i]/3)][dummy[i]%3].innerHTML = '<div class="letter">x</div>';
      dummy.splice(i,1);
